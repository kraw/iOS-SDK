...
//  self.naviginePedometer.text = [NSString stringWithFormat:@"Navigine:%zd length:%lf",res.outStepCounter,res.outStepLength];
  ...
  if(enableFollow ){
    ...
    self.contentView.transform = CGAffineTransformMakeRotationAtPoint(-res.Yaw, rPoint, zoomScale);
  }
...
CGAffineTransform CGAffineTransformMakeRotationAtPoint(CGFloat angle, CGPoint pt, CGFloat scale){
...
-(void) changeFloorTo:(NSInteger)row{
  ...
  if (enableFollow){
    ...
    self.contentView.transform = CGAffineTransformMakeRotationAtPoint(0.0, point,zoomScale);
  }
